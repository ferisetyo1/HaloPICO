...
class SelfReportCovidActivity : AppCompatActivity() {
    ...
    private fun updateSoal(
        ...
        if (listSoalModel.size <= counter) {
            ...
            val docData = hashMapOf(
                "statusData" to 2,
                "selfReportCovid" to listJawaban,
                "selfReportCovidHasil" to cekSelfReportResult()
            )
            mDB.runBatch {
                ...
                dialog.dismiss()
                if (it.isSuccessful) {
                    startActivity(Intent(this, ThankYou::class.java).putExtra("isCovid", true))
                    finish()
                }
            }
        } else {
            ...
            listSoalModel.get(counter).apply {
                ...
                jawaban.forEach {
                    val jawaban = it
                    val button = MaterialButton(this@SelfReportCovidActivity).apply {
                        cornerRadius = 25
                        text = it.jawaban
                        setBackgroundColor(
                            ContextCompat.getColor(
                                this@SelfReportCovidActivity,
